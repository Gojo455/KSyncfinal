...
app = Flask(__name__)
app.secret_key = os.environ.get('cinema_SECRET', 'abujacine-stable-secret-key-2026-do-not-share')
...
def compute_seat_quality(row, col, total_rows, total_cols):
    ...
    Uses a 2-D Gaussian-like decay from the research-derived optimal point:
      - Optimal row  : 55 % back  (SMPTE / THX sweet zone)
      - Optimal col  : 50 % (dead centre, satisfies ITU-R ≤ 30° angle)

    Row decay is asymmetric (exponent 1.4) so the score drops faster toward
    the front (neck strain) than toward the rear (mild perspective loss).
    Column decay is symmetric (exponent 2, standard Gaussian).

    Row contributes 60 %, column 40 %, reflecting that longitudinal distance
    from the screen affects perception more than lateral offset.
    """
    rn = row / total_rows   # normalised: 0 = front row, 1 = back row
    cn = col / total_cols   # normalised: 0 = far left, 1 = far right

    # Distance from the research-defined optimum
    opt_r, opt_c = 0.55, 0.50
    rd = abs(rn - opt_r)
    cd = abs(cn - opt_c)

    # Decay curves — further from optimum = lower score
    rs = math.exp(-3.2 * rd ** 1.4)   # asymmetric in the row direction
    cs = math.exp(-2.8 * cd ** 2)     # symmetric in the column direction

    # Research-backed penalties
    if rn < 0.18: rs *= 0.45   # Front rows: severe neck-strain zone (ergonomics)
    if rn > 0.88: rs *= 0.80   # Very back: slight audio/perspective degradation
